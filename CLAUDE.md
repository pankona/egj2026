...
ドラッグで「方向と起点」を入力すると、指を離した瞬間にドラッグ開始点から方向へ**光の衝撃波**がズバッと伸びる。ドラッグ距離を 3 段階に量子化して 1〜3 ユニットのエネルギーを消費 (短く引けば短い線、大きく引けば長い線)。バッファ容量は 6 ユニットでトリクル回復、3 unit 大ぶり 2 連発も 1 unit 小撃ち連射も両立する。3 連発した直線でポリゴン状に囲んだ暗領域は一気に光で塗りつぶされる（claim）。敵は光の縁を侵食しながら、複数体が連携して囲い返してくる。**ステージの敵を全滅させると次へ**。最終 10 ステージは巨大ボスが claim 成立ごとに分裂する持久戦で (フル → ハーフ ×2 + 雑魚 ×2 → さらに雑魚 ×2)、最終的にすべての破片を封じるとクリア。
...
| クリア        | クリック / Space で次ステージへ (mandala アニメ中は入力無視)                  |
...
初期実装の最終ステージ (`Stage.Boss = true`) は「1 回の claim で巨大ボスを封じたら終わり」だった。`Stage.Enemies = 1` の弱小ステージと判定ロジックが同一なので、ある程度上手いプレイヤーには最終戦が雑魚 1 体と同じ温度感になってしまっていた。テーマ DISCONNECT を最終戦で体現するため、「光で封じるたびにボスが**切断**されて小さくなる」分裂式に変えた。

- **3 段階の HP**: フルボス (`Radius = BossFullRadius = 70`) → ハーフボス ×2 (`HalfBossRadius = 38`) → 雑魚 (`BossZakoRadius = 14`)。各ボス系エネミーは 1 回の claim でしか倒せず、`Enemy.SplitsLeft` で残り分裂回数を持つ (フル = 2, ハーフ = 1, 雑魚 = 0)。雑魚は通常敵と同じ 1 撃即死
- **claim ごとに 4 体に増える**: `spawnBossSplits` がフル → ハーフ ×2 + 雑魚 ×2、ハーフ → 雑魚 ×2 を生成する。最初の claim で 1 → 4 体、両ハーフを同時 claim できれば次は 4 → 4 体 (-2 ハーフ + 4 雑魚)、最終的に全雑魚を一網打尽すれば終わり。**理論最短 3 claim、平均 5〜10 claim 想定**
- **湧き出し位置 = 暗領域のみ (`pickDarkSpawn`)**: ボスを claim した瞬間、その閉領域はすでに `Light = 1` で塗られている。素朴に親の位置で湧かせるとプレイヤーは「囲んだはずの中から子が出てきた」感覚になり儀式の手応えが消える。`pickDarkSpawn` は magic circle 内の `Cell.Light < WallLightThreshold` セルだけを候補にして抽選するので、子は必ずまだ暗い領域 = 次の攻略対象に出現する。ハーフは親から `HalfBossSpawnMinDistPx = 70`、雑魚は `BossZakoSpawnMinDistPx = 110` の距離を要求して、画面全体に散らばるように
- **雑魚は通常より速い (`BossZakoSpeedMul = 2.0`)**: 数で押し寄せる代わりに侵食半径は通常敵と同じに保ち、速度だけ 2 倍に。「ぐずぐずしてると囲い返される」テンポを最終戦だけ強める
- **`claimEnclosure` のループ書き換え**: 旧実装は「sealed されたエネミーをスライスから消す」だけだった。分裂を入れたので `for _, e := range g.enemies` で sealed されたか個別判定し、`SplitsLeft > 0` なら `spawnBossSplits` の結果を追加して残す代わりに本体を消す。SEALED! フラッシュと SFX の発火条件を `len(g.enemies) < before` から `kills > 0` に変更したのは、分裂で count が増えてもプレイヤーには「ボスを封じた」リワードを返したいため

旧仕様で残っている前提の整理: `advanceBindPhase` の `s.Boss` ガードは「結界フェーズを最終ステージで動かさない」目的なのでそのまま (途中で複数体になっても同じ意味で残す)。勝利条件 `len(g.enemies) == 0` は無変更 — 雑魚を全滅させれば自動で成立する。
...
// ラストボスの分裂
BossInitialSplits      = 2     // フルボスが claim ごとに分裂する回数 (2 → 1 → 0)
BossFullRadius         = 70.0  // フルボスの描画半径
BossFullEffectRadius   = 56.0  // フルボスの侵食半径
HalfBossRadius         = 38.0  // ハーフボスの描画半径
HalfBossEffectRadius   = 30.0  // ハーフボスの侵食半径
BossZakoRadius         = 14.0  // 分裂後に湧く雑魚の描画半径
BossZakoSpeedMul       = 2.0   // 雑魚は stage.EnemySpeed の何倍で動くか
HalfBossSpawnMinDistPx = 70.0  // ハーフボスを親からどれだけ離して湧かせるか
BossZakoSpawnMinDistPx = 110.0 // 雑魚を親からどれだけ離して湧かせるか
```
...
- **ラストボス分裂のテンポ**: 平均 5〜10 claim 想定だが、雑魚速度 2 倍 + magic circle 内に湧くので「囲む前にぐるぐる逃げ回って詰む」可能性あり。キツすぎなら `BossZakoSpeedMul` を 1.5 に、緩すぎなら `BossInitialSplits` を 3 にしてフェーズを足す。湧き位置が遠すぎて「どこに出たか分からない」感が出るようなら `BossZakoSpawnMinDistPx` を 80 に下げる
- **ラストボス分裂時の SE / 視覚演出**: 現状は通常の `sealedFlashFrames` + `sfxEnemyDown` のみで「分裂したぞ」感が薄い。分裂専用の SFX や、親が消えた位置から子の湧き位置へ繋がるフラッシュトレイルがあると「切断」のテーマ的にハマる
- **`pickDarkSpawn` のフォールバック挙動**: 候補ゼロ (画面全体が光ってる極端なケース) のときは親の位置にそのまま湧かせている。理論上は最終盤でしか起きないが、起きれば「湧いた瞬間に隣接 claim でまた消える」連鎖になり得る。気になるなら `Light` しきい値を緩める or magic circle 制約を外す段階フォールバックを足す
