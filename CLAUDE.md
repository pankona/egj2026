...
- 領域確定 (`claimEnclosure`): 斬撃が全長まで伸びた瞬間に外周→外側を flood-fill。閉じた暗領域を見つけたら現在の hue で塗り、内部の敵は除去
...
- **claim は既存の flood-fill を流用**: 斬撃の光と画面端が壁になり、自然に閉領域ができる。新しい閉領域判定ロジックは追加せず、`fireSlash` ではなく「斬撃が全長まで伸びた瞬間」(`updateSlashes` 内) で `claimEnclosure()` を呼ぶ
